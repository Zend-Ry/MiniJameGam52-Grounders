...
using DebugUtility = Unity.FPS.Game.DebugUtility;
...
namespace Unity.FPS.Gameplay
{
    ...
    public class PlayerCharacterController : MonoBehaviour
    {
        ...
        [Tooltip("distance at which the player can interact with objects in the world by looking at them")]
        public float InteractDistance = 3f;
        
        [Tooltip("Current raycastHit from looking at interactable objects")]
        public RaycastHit LookAtHit { get; private set; }
        ...
        [Tooltip("Multiplicator for the walk speed (based on grounded speed)")]
        ...
        [Tooltip("Amount of footstep sounds played when moving one meter while walking")]
        public float FootstepSfxFrequencyWhileWalking = 0.5f;
        ...
        void Update()
        {
            ...
            LookAtHit = InteractionRaycast();
            
            if(m_InputHandler.GetInteractInputDown() && LookAtHit.collider != null)
            {
                if (LookAtHit.collider.GetComponentInParent<IInteractable>() is IInteractable interactable)
                {
            ...
            // crouching
            if (m_InputHandler.GetSneakInputDown())
            {
                SetCrouchingState(!IsCrouching, false);
            }

            if (m_InputHandler.GetRunWalkInput())
            {
                IsWalking = !IsWalking;
            }
        ...
        {
            Physics.Raycast(PlayerCamera.transform.position, PlayerCamera.transform.forward, out RaycastHit LookAt, InteractDistance);
        ...
        void HandleCharacterMovement()
        {
            ...
            bool isSprinting = m_InputHandler.GetSprintInputHeld();
            {
                ...
                float speedModifier = isSprinting ? SprintSpeedModifier : IsWalking ? WalkSpeedModifier : 1f;
                ...
                if (IsGrounded)
                {
                    ...
                    float chosenFootstepSfxFrequency =
                        (isSprinting ? FootstepSfxFrequencyWhileSprinting :
                            IsWalking ? FootstepSfxFrequencyWhileWalking : FootstepSfxFrequency);
